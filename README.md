...
> ⚠️ **데이터 안내**: 이 저장소의 `data/` 파일(거래처·이력·기온)은 **실제 데이터가 아니라 스키마만 동일한 가상(합성) 샘플**입니다. `python scripts/generate_sample_data.py`로 언제든 재생성할 수 있습니다. 학습된 모델(`python/models/*.pkl`)과 리포트 파일은 저장소에 포함하지 않으며, 아래 학습 단계를 실행하면 샘플 데이터로 생성됩니다.
