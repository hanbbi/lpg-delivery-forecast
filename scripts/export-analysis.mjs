 ...
 *
 * 실행: node scripts/export-analysis.mjs
 */
...
async function main() {
  ...
  for (let i = 0; i < customers.length; i++) {
    ...
    results.push({
      ...
      충전횟수: chargeEvents.length,
      사용모델: modelType,
      평균충전주기_일: Math.round(avgInterval * 10) / 10,
      주기표준편차_일: Math.round(stdInterval * 10) / 10,
      평균충전시작잔량_퍼센트: Math.round(avgRemain * 10) / 10,
      최소잔량기록_퍼센트: Math.round(minRemain * 10) / 10,
      현재잔량_퍼센트: Math.round(currentRemain * 10) / 10,
      마지막충전일: lastChargeDate,
      마지막충전후경과_일: daysSinceCharge,
      다음충전예측_일: Math.round(predictedDays * 10) / 10,
      규칙기반예측_일: Math.round(ruleBasedEstimate * 10) / 10,
      잔량기반예측_일: Math.round(remainBasedEstimate * 10) / 10,
      일일소비율_퍼센트: Math.round(dailyConsumption * 10) / 10,
      평균기온_도: Math.round(avgTemp * 10) / 10,
      계절: season,
      주말여부: isWeekend,
    });
  }
  ...
  if (validResults.length > 0) {
    ...
    const avgCycle = validResults.reduce((sum, r) => sum + (r['평균충전주기_일'] || 0), 0) / validResults.length;
